...
export const ClassNote = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!form.image || !form.class || !form.subject || !form.year || !form.term || !form.topic) {
  ...
  useEffect(() => {
    ...
    const isComplete = form.subject && form.class && form.year && form.image && form.term && form.topic;
    setActive(!!isComplete);
    if (!form.image) setIsChoosen(false);
  }, [form]);

  return (
    <form onSubmit={handleSubmit} className="md:flex md:justify-between md:gap-8 md:flex-row flex-col w-full">
      <section className="w-full flex-1">
        ...
        <div className="space-y-4">
          ...
          <FormSelect label="Year" options={Years} onChange={handleSelectChange('year')} />
          <FormSelect label="Term" options={Terms} onChange={handleSelectChange('term')} />
        </div>
      ...
      <section className="w-full flex-1 mt-6 md:mt-0">
        <FormInput label="Header" name="topic" value={form.topic} onChange={handleInputChange} />
        <div className="mb-4">
        ...
        <label className="relative w-full h-60 border border-[#667085] xl:w-[430px] rounded-lg hover:border-[#F6C354] flex items-center justify-center text-center p-4 cursor-pointer">
          ...
          <div className="flex flex-col items-center gap-2 text-[#475467] pointer-events-none">
            ...
            <p className="font-medium">Drag and drop video here</p>
            <p className="text-sm text-[#667085]">or</p>
            <button type="button" className="px-4 py-2 bg-orange-500 text-white rounded-md text-sm hover:bg-orange-600 transition">
              Choose File
            </button>
...
const FormInput = ({ label, name, value, onChange }: any) => (
  <div className="flex flex-col gap-2 w-full mb-4">
    ...
    <input
      ...
      value={value}
      onChange={onChange}
      className="w-full p-3 xl:w-[430px] hover:border-[#F6C354] rounded-lg border border-[#667085]"
    />
  </div>
);

const FormSelect = ({ label, options, onChange }: any) => (
  <div className="flex flex-col gap-2 w-full mb-4">
    <label className="text-[#344054]">{label}</label>
    <Select onValueChange={onChange}>
      <SelectTrigger className="w-full p-3 xl:w-[430px] hover:border-[#F6C354] rounded-lg border border-[#667085]">

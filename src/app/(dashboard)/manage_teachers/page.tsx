...
export default function page () {
    ...
    useEffect( ()=>{
      const fetchData = async ()=>{
        try {
          const response = await fetch('https://citadel-i-project.onrender.com/api/v1/admin/get_tutors', {
            method: 'GET',
            headers: {
    ...
    return(
      <section className={`w-ful px-[16px] pb-[24px] mt-6 min-h-screen ${sidebarOpen && "hidden md:block"}`}>
         ...
         <section className="grid md:grid-cols-1 lg:grid-cols-3 gap-[24px] mt-10">
          ...
          <div className='bg-[#FFFFFF] p-[16px] gap-[10px] rounded-[8px] flex flex-col items-center justify-between'>
          ...
          <div className="flex flex-col md:flex-row lg:flex-col gap-[8px]">
                  <div className='bg-[#FFFFFF] p-[16px] gap-[10px] w-full rounded-[8px] flex items-start justify-between'>
                    <div className='flex flex-col gap-[16px]'>
                      ...
                      <div className='flex items-center gap-[8px]'>
                          <p className='text-[#0F0F0F] text-[24px] font-[700]'>10</p> 
                          <div className='flex text-[#0DAF64] text-[12px] '>
                             <FaArrowUp/>
                             <p>12%</p>
                          </div>
                  ...
                  <div className='bg-[#FFFFFF] p-[16px] gap-[10px] w-full rounded-[8px] flex items-start justify-between'>
                    <div className='flex flex-col gap-[16px]'>
                      <p>New sign up this week</p>
                      <div className='flex items-center gap-[8px]'>
                          <p className='text-[#0F0F0F] text-[24px] font-[700]'>4</p> 
                          <div className='flex text-[#0DAF64] text-[12px] '>
                             <FaArrowUp/>
                             <p>0.2%</p>
                          </div>
                        </div>
                      <p className='text-[#0DAF64] text-[12px] '>New</p>
                    </div>
                    <div>
                    <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect width="40" height="40" rx="20" fill="#FFEEE6"/>

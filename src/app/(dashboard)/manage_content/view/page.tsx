...
const id = idString ? Number(idString) : undefined; // or null, or 0, depends on your needs
    return(
        <section className={`w-ful px-[16px] pb-[24px] mt-6 gap-[16px] flex flex-col min-h-screen ${sidebarOpen && "hidden md:block"}`}>
            ...
            <main className="p-[32px] bg-white rounded-[8px] gap-[160px] flex justify-between overflow-x-hidden">
               {
                  type === 'Class Note' && id !== undefined && <ViewClassNote id={id} />

               }
                {
                  type === 'Exam Question' && id !== undefined && <ViewExamQuestion id={id} />
               }
                {
